...
def _apply_host_ca_bundle(url: str, kwargs: dict) -> dict:
    """Use an OS-exported CA bundle only for an intercepted chatgpt.com connection."""
    if "verify" in kwargs:
        return kwargs
    try:
        ...
        return kwargs
    bundle = str(os.getenv("CHATGPT_CA_BUNDLE", "") or "").strip()
    if host == "chatgpt.com" and bundle:
...
class BrowserSession:
    ...
    def get(self, url: str, headers: dict = None, **kwargs):
        ...
        kwargs = _apply_host_ca_bundle(url, kwargs)
