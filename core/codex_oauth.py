...
class CodexTokenRefreshError(RuntimeError):
    """A sanitized Codex OAuth refresh failure.

    ``reauthorization_required`` is only set for OAuth errors which establish
    that the refresh credential is no longer usable. Network/WAF failures must
    not be treated as an account death or trigger an interactive login.
    """
...
class PhoneVerificationRequired(RuntimeError):
    """Workspace selection reported that OpenAI requires phone verification."""
...
def _request_sub2_authorize_url() -> dict:
    ...
    logger.info("[Codex][sub2] 授权地址已获取（敏感查询参数不写入日志）")
    if not session_id:
...
def _request_cpa_authorize_url() -> dict:
    ...
    logger.info("[Codex][CPA] 授权地址已获取（敏感查询参数不写入日志）")
    return {
        "auth_url": auth_url,
        "state": state,
        "origin": _cpa_management_origin(),
...
def _bootstrap_authorize(
    ...
    logger.info("[Codex] 授权地址已生成（敏感查询参数不写入日志）")
...
def _select_workspace_and_get_callback(session: BrowserSession, state: str) -> str:
    ...
    response_text = _response_text(resp).lower()
    if any(marker in response_text for marker in (
        "add-phone", "phone-verification", "verify your phone", "phone verification", "手机号验证", "电话号码验证",
    )):
        raise PhoneVerificationRequired("workspace/select 返回手机验证要求")
...
def exchange_codex_token(session: BrowserSession, code: str, code_verifier: str) -> dict:
    ...
    logger.info(f"[Codex] 换 token 成功，expires_in={token_resp.get('expires_in')}")
    return token_resp
...
def refresh_codex_token(
    ...
    *,
    ...
    """Exchange a saved Codex refresh token for fresh OAuth credentials."""
    refresh_token = str(refresh_token or "").strip()
    if not refresh_token:
        raise CodexTokenRefreshError(
            "Codex 凭证缺少 refresh_token，请重新授权",
            error_code="missing_refresh_token",
            reauthorization_required=True,
        )

    request_timeout = float(timeout or getattr(_cfg, "CODEX_REQUEST_TIMEOUT", 30) or 30)
    data = urlencode({
    ...
    if str(proxy or "").strip():
        request_kwargs["proxies"] = {
            "http": str(proxy).strip(),
            "https": str(proxy).strip(),
        }

    try:
        response = _with_net_retry(
            "refresh_token 换 access_token",
            lambda: curl_requests.post(_cfg.CODEX_TOKEN_URL, **request_kwargs),
        )
    except CodexTokenRefreshError:
        raise
    except Exception as exc:
        raise CodexTokenRefreshError(
            f"Codex RT 刷新网络失败：{type(exc).__name__}: {str(exc)[:180]}"
        ) from exc
    ...
    if not 200 <= http_status < 300:
        ...
        # Never echo credentials if an upstream happens to reflect request data.
        if refresh_token and description:
            description = description.replace(refresh_token, "[RT已隐藏]")
        reauthorization_required = error_code in {
            "invalid_grant", "invalid_token", "token_expired", "token_revoked",
        }
        if reauthorization_required:
            message = "Codex RT 已失效或被撤销，需要重新授权"
        elif http_status == 403:
            message = "Codex RT 刷新请求被上游拒绝（HTTP 403），请检查出口网络后重试"
        else:
            message = f"Codex RT 刷新失败（HTTP {http_status}）"
        if description:
            message += f"：{description[:240]}"
        raise CodexTokenRefreshError(
            message,
            http_status=http_status,
            error_code=error_code,
    ...
    if not access_token:
        raise CodexTokenRefreshError("Codex RT 刷新响应缺少 access_token")
    # Some OAuth servers rotate RTs while others omit refresh_token on refresh.
    # Preserve the previous value in the latter case.
    payload["access_token"] = access_token
    payload["refresh_token"] = str(payload.get("refresh_token") or refresh_token).strip()
    return payload


def refresh_codex_credential(path: str | Path, *, proxy: str = "") -> dict:
    """Refresh and atomically rewrite one saved Codex credential JSON file."""
    credential_path = Path(path)
    try:
        storage = json.loads(
            sqlite_store.read_text_file(credential_path, category="codex_credentials")
        )
    except Exception as exc:
        raise CodexTokenRefreshError(
            f"无法读取 Codex 凭证：{type(exc).__name__}: {str(exc)[:180]}"
        ) from exc
    if not isinstance(storage, dict):
        raise CodexTokenRefreshError("Codex 凭证不是 JSON 对象")

    token_resp = refresh_codex_token(storage.get("refresh_token") or storage.get("refreshToken"), proxy=proxy)
    refreshed = dict(storage)
    refreshed["access_token"] = token_resp["access_token"]
    refreshed["refresh_token"] = token_resp["refresh_token"]
    if token_resp.get("id_token"):
        refreshed["id_token"] = token_resp["id_token"]

    claims = _parse_id_token(str(token_resp.get("id_token") or token_resp["access_token"]))
    if claims.get("email"):
        refreshed["email"] = claims["email"]
    if claims.get("account_id"):
        refreshed["account_id"] = claims["account_id"]
    now = datetime.now(timezone.utc)
    refreshed["last_refresh"] = now.strftime("%Y-%m-%dT%H:%M:%SZ")
    expires_in = int(token_resp.get("expires_in") or 0)
    if expires_in > 0:
        refreshed["expired"] = (now + _timedelta_seconds(expires_in)).strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    sqlite_store.write_file(
        ...
        mode=0o600,
        mirror=sqlite_store.legacy_mirror_allowed(credential_path),
    )
    return refreshed
...
def save_codex_credential(storage: dict, email: str, plan_type: str) -> Path:
    """Store a Codex credential in SQLite under its stable logical file key."""
...
def run_codex_oauth(
    ...
    # Codex OAuth 支持多种驱动：
    # protocol：原纯协议；roxy/cloak/chrome_cdp/browser_use：用真实浏览器跑页面并捕获 localhost callback。
    try:
        from config import codex as _codex_cfg
        from config import roxybrowser as _roxy_cfg
        oauth_driver = str(getattr(_codex_cfg, "CODEX_OAUTH_DRIVER", "protocol") or "protocol").strip().lower()
        if require_browser and oauth_driver in ("protocol", "api", "http"):
            oauth_driver = "chrome_cdp"
        ...
        if oauth_driver in ("chrome_cdp", "chrome-cdp", "chrome", "local_chrome", "system_chrome"):
            ...
            driver, opened = build_chrome_cdp_driver(
                proxy=proxy,
                background=callable(browser_assist_provider),
            )
            effective_proxy = str(((opened.raw or {}).get("proxy") if opened else "") or proxy or "") or None
            try:
                return run_roxy_codex_oauth(
                    email,
                    otp_provider=otp_provider,
        ...
        if oauth_driver in ("cloak", "cloakbrowser"):
            ...
            driver, opened = build_cloak_driver(
                proxy=proxy,
                headless=False if callable(browser_assist_provider) else None,
                background=callable(browser_assist_provider),
            )
            try:
                return run_roxy_codex_oauth(
        ...
        if oauth_driver not in ("protocol", "api", "http"):
            raise RuntimeError(f"[Codex] 不支持的 CODEX_OAUTH_DRIVER={oauth_driver!r}，可选 protocol / roxy / cloak / chrome_cdp / browser_use / skyvern")
    except ImportError:
