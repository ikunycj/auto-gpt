...
def _detect_browser_kind(opened=None) -> str:
    try:
        ...
        if isinstance(raw, dict):
            driver_name = str(raw.get("driver") or "").lower()
            if driver_name.startswith("cloak"):
                return "Cloak"
            if driver_name in {"chrome_cdp", "chrome-cdp", "system_chrome"}:
                return "ChromeCDP"
    except Exception:
        pass
    return "Roxy"
...
def _is_cloudflare_challenge(driver) -> bool:
    """Return whether the current document is a Cloudflare security interstitial."""
    try:
        url = str(getattr(driver, "current_url", "") or "").lower()
        title, text = driver.execute_script(
...
def _background_browser_window(driver) -> None:
    try:
        driver.minimize_window()
    except Exception as exc:
        logger.debug("[Codex][Browser] 浏览器窗口转入后台失败：%s", str(exc)[:160])


def _focus_browser_window(driver) -> None:
    try:
        focus = getattr(driver, "focus_window", None)
        if callable(focus):
            focus()
            return
        ...
        raise RuntimeError(f"无法激活对应浏览器窗口：{type(exc).__name__}: {exc}") from exc


def _call_browser_assist(
    provider,
    reason: str,
    url: str,
    resolved_check=None,
    ...
    """Call newer assist providers while preserving legacy two/three-argument callbacks."""
    try:
        ...
        if accepts_varargs or len(positional) >= 5:
            return provider(reason, url, resolved_check, focus_action, background_action)
        if callable(resolved_check) and len(positional) >= 3:
            return provider(reason, url, resolved_check)
    except (TypeError, ValueError):
        pass
    return provider(reason, url)
...
def _wait_for_callback(driver, timeout: int | None = None) -> str:
    ...
    while time.time() < end:
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"等待 Codex callback 超时，最后 URL={_safe_diagnostic_url(last_url)}")
...
def _page_text(driver) -> str:
    try:
        return str(driver.execute_script("return document.body ? document.body.innerText : ''") or "").lower()
    except Exception:
        return ""
...
def _login_step_state(driver) -> str:
    """Return a non-password state reached after submitting the email.

    Auth pages can transition directly to an email OTP, 2FA, phone, or local
    callback page.  In those cases a password input is not expected and must
    not be treated as a failed password submission.
    """
    try:
    ...
    if _is_email_verification_page(driver):
        return "email_otp"
    if any(x in lower_url for x in ("/phone", "/workspace", "/consent", "/authorize")):
        # The initial OAuth URL also contains /authorize.  Only accept it as a
        # post-login state once the page no longer exposes an email field.
        try:
            state = _email_otp_page_state(driver)
            attrs = " ".join(
            ...
            if "email" in attrs or "username" in attrs:
                return ""
        except Exception:
            if "/authorize" not in lower_url:
                return "next"
        return "next"
...
def _submit_login_password(driver, password: str, timeout: int = 25) -> str:
    """Fill the existing-account password step, or report an already-reached challenge.

    Returns ``submitted`` for a password submission, ``email_otp`` when the
    flow has already switched to email verification, ``challenge`` for a 2FA
    page, and ``next`` for a later OAuth/phone/callback page.
    """
    end = time.time() + timeout
    last_error = None
    selectors = [
    ...
    while time.time() < end:
        try:
            state = _login_step_state(driver)
            if state:
                logger.info("[Codex][Browser] 密码输入框出现前已进入后续页面：%s", state)
                return state
            field = _find_any(driver, selectors, timeout=2)
            field.clear()
            ...
            if not _click_if_present(driver, [
                ...
                raise RuntimeError("未找到密码页提交按钮")
            logger.info("[Codex][Browser] 已提交账号密码")
            return "submitted"
        except Exception as exc:
            last_error = exc
            if passwordless_retries < 2:
                try:
                    ...
                    if result.get("ok"):
                        passwordless_retries += 1
                        logger.info("[Codex][Browser] 密码输入框未出现，已点击一次性验证码入口：%s", result.get("reason"))
                        human_delay("form")
                except Exception as passwordless_exc:
                    logger.debug("[Codex][Browser] 一次性验证码入口探测失败：%s", str(passwordless_exc)[:140])
            time.sleep(0.6)
    state = _login_step_state(driver)
    if state:
        logger.info("[Codex][Browser] 密码页等待结束时已进入后续页面：%s", state)
        return state
    try:
        current_url = _safe_diagnostic_url(str(getattr(driver, "current_url", "") or ""))
        page_text = _page_text(driver)[:240]
    except Exception:
        current_url, page_text = "", ""
    raise RuntimeError(
        "等待或提交密码页超时："
        f"{str(last_error or '')[:160]}；url={current_url}；页面文本={page_text}"
    )


def _post_password_challenge(driver, totp_provider, timeout: int = 25) -> str:
    """Handle TOTP when shown; otherwise tell the caller whether email OTP is needed."""
    end = time.time() + timeout
    while time.time() < end:
        url = str(getattr(driver, "current_url", "") or "").lower()
        text = _page_text(driver)
...
def _fill_email_and_otp(
    ...
    totp_provider=None,
    browser_assist_provider=None,
) -> None:
    otp_after_ts = time.time()
    logger.info("[Codex][Browser] 打开授权地址")
    logger.info("[Codex][Browser] 授权地址已生成（敏感查询参数不写入日志）")
    driver.get(auth_url)
    ...
    if _is_cloudflare_challenge(driver):
        if not callable(browser_assist_provider):
            raise RuntimeError("Cloudflare 浏览器验证未完成，请使用可见浏览器完成验证后重新授权")
        _call_browser_assist(
            browser_assist_provider,
            "cloudflare",
            str(getattr(driver, "current_url", "") or ""),
            resolved_check=lambda: not _is_cloudflare_challenge(driver),
            focus_action=lambda: _focus_browser_window(driver),
            background_action=lambda: _background_browser_window(driver),
        )
        if _is_cloudflare_challenge(driver):
            raise RuntimeError("人工协助后浏览器仍停留在 Cloudflare 验证页")
    ...
    if login_password:
        logger.info("[Codex][Browser] 已提交邮箱，等待已有账号密码页")
        password_state = _submit_login_password(driver, login_password)
        if password_state == "email_otp":
            logger.info("[Codex][Browser] 密码页未出现，当前已进入邮箱 OTP 页面")
        elif password_state == "next":
            return
        else:
            if _post_password_challenge(driver, totp_provider) == "next":
                return
    else:
    ...
    for otp_attempt in range(1, max_otp_attempts + 1):
        ...
        logger.info("[Codex][Browser] 已收到邮箱 OTP")
...
def _wait_for_fresh_email_otp(otp_provider, email: str, after_ts: float, used_codes: set[str] | None = None, timeout: int = 90) -> str:
    ...
    while True:
        ...
        if remaining <= 0:
            raise RuntimeError("等待新的邮箱验证码超时，取码接口仍返回已失败验证码")
        logger.warning(
            "[Codex][Browser] 取码接口仍返回已提交过的旧 OTP，继续等待最新验证码（剩余 %ss）",
            remaining,
        )
...
def _do_phone_verification_if_present(driver) -> bool:
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                already_sent = direct_code_page or _is_phone_code_page(driver)
                if not already_sent:
                    ...
                    logger.info("[Codex][Browser] 已在手机验证码页，复用当前绑定手机号等待短信，不重新提交新号码")
                ...
                logger.info("[Codex][Browser] 已收到手机 OTP")
                ...
                if activation_id:
                    try:
                        ...
                        pass
                if direct_code_page:
                    raise RuntimeError("已绑定手机号的短信验证失败，未改用新号码；请检查该手机号或短信 URL") from exc
...
def _run_roxy_codex_oauth_once(
    ...
    try:
        ...
        if callable(browser_assist_provider):
            _background_browser_window(driver)
        browser_timeout = int(_roxy_cfg.ROXY_SELENIUM_TIMEOUT)
        if _detect_browser_kind(opened) == "Cloak":
            try:
                from config import cloakbrowser as _cloak_cfg
                browser_timeout = int(getattr(_cloak_cfg, "CLOAK_SELENIUM_TIMEOUT", browser_timeout) or browser_timeout)
            except Exception:
                pass
        ...
        logger.info("[Codex][Browser] 已捕获 OAuth callback code")
        ...
        if id_claims.get("email") and effective_email.strip().lower() != email.strip().lower():
            raise RuntimeError("OAuth 最终授权账号与目标邮箱不一致，已拒绝保存凭证")
