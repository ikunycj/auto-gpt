...
class CloakElement:
    ...
    def send_keys(self, *values: str) -> None:
        ...
        if text in special_keys:
            ...
            return
        # Selenium send_keys 是在当前光标位置追加输入；fill() 会每次覆盖整个值。
        # 人类化输入会逐字符调用 send_keys，因此必须使用键盘输入保留前文。
...
class CloakSeleniumDriver:
    ...
    def current_window_handle(self) -> str:
        pages = self._pages()
        try:
            return str(pages.index(self.page))
        except ValueError:
            return "0"
    ...
    def _set_window_state(self, state: str) -> bool:
        try:
            info = self.execute_cdp_cmd("Browser.getWindowForTarget", {}) or {}
            window_id = info.get("windowId")
            if window_id is None:
                return False
            self.execute_cdp_cmd("Browser.setWindowBounds", {
                "windowId": window_id,
                "bounds": {"windowState": state},
            })
            return True
        except Exception as exc:
            logger.debug("[Cloak] 设置浏览器窗口状态失败 state=%s: %s", state, exc)
            return False
...
def build_cloak_driver(
    ...
    headless_value = bool(getattr(_cfg, "CLOAK_HEADLESS", False)) if headless is None else bool(headless)
    if background and not headless_value and "--start-minimized" not in launch_args:
    ...
    if background and not headless_value:
