...
def upsert_sub2api_account(
    ...
    *,
    ...
    def mutate(current: Any) -> dict[str, Any]:
        ...
        # Copy the list so a failed callback never mutates the value held by a
        # caller or a decoded SQLite row before the transaction commits.
    ...
    updated = bool(operation["updated"])
    return {
        "ok": True,
        "updated": updated,
        "added": not updated,
...
def create_codex_agent_identity(
    ...
    sqlite_store.write_file(
        output_path,
        json.dumps(auth_json, indent=2, ensure_ascii=False) + "\n",
        category="agent_credentials",
        mode=0o600,
        mirror=False,
    )
