...
def _fixed_sms_code(text: str) -> str:
    """Extract a six-digit code from a fixed SMS page response."""
    body = str(text or "")
    context_matches = re.findall(
        r"(?:code|验证码|校验码|verification)\D{0,80}(\d{6})",
        body,
        flags=re.IGNORECASE,
    )
    if context_matches:
...
def acquire_number(
    ...
    try:
        ...
        if _provider() == "fixed_url":
            ...
            acquire_provider = override.get("acquire_provider")
            if callable(acquire_provider):
                material = acquire_provider() or {}
                if not isinstance(material, dict):
                    raise SmsProviderError("延迟取号提供器返回格式错误")
                phone = "".join(ch for ch in str(material.get("phone") or "") if ch.isdigit())
                code_url = str(material.get("code_url") or "").strip()
                activation_id = str(material.get("activation_id") or "").strip()
                override["phone"] = phone
                override["code_url"] = code_url
                if not activation_id:
                    activation_id = "fixed-" + hashlib.sha256(f"{phone}\n{code_url}".encode()).hexdigest()[:16]
            else:
                phone, code_url = _fixed_sms_config()
                activation_id = "fixed-" + hashlib.sha256(f"{phone}\n{code_url}".encode()).hexdigest()[:16]
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            if provider == "fixed_url":
                override = _FIXED_SMS_OVERRIDE.get() or {}
                code_provider = override.get("code_provider")
            ...
            if provider == "l":
                ...
                if code:
                    logger.info(f"[SMS:L] 第 {round_no} 轮收到验证码")
                    return code
                remaining = max(0, int(deadline - time.time()))
                logger.info(
            ...
            if provider == "h":
                ...
                status = str((data.get("item") or {}).get("status") or "").strip()
                if code:
                    logger.info(f"[SMS:H] 第 {round_no} 轮收到验证码")
                    return code
                remaining = max(0, int(deadline - time.time()))
                logger.info(
            ...
            if text.startswith("STATUS_OK:"):
                ...
                logger.info(f"[SMS] 第 {round_no} 轮收到验证码")
                return code
