...
def _window_kind(window: dict | None) -> str:
    try:
        seconds = float((window or {}).get("limit_window_seconds") or 0)
    except (TypeError, ValueError):
        return ""
    if seconds <= 0:
        return ""
    if 28 * 24 * 60 * 60 <= seconds <= 31 * 24 * 60 * 60:
        return "monthly"
    return "weekly" if seconds >= 24 * 60 * 60 else "short"


def _usage_windows(data: dict, rate_limit: dict) -> list[dict]:
    windows: list[dict] = []

    def append_rate_limit(value: object) -> None:
    ...
    append_rate_limit(rate_limit)
    additional = data.get("additional_rate_limits") or []
    if isinstance(additional, list):
        for item in additional:
            if not isinstance(item, dict):
                continue
            append_rate_limit(item.get("rate_limit"))
            append_rate_limit(item)
    return windows


def parse_codex_usage(data: dict, *, token: str = "") -> dict:
    """Extract the Codex short, weekly, and monthly windows from wham/usage."""
    if not isinstance(data, dict):
        raise ValueError("Codex 用量响应不是 JSON 对象")
    claims = token_claims(token) if token else {}
    rate_limit = data.get("rate_limit") or {}
    if not isinstance(rate_limit, dict):
        raise ValueError("Codex 用量响应缺少 rate_limit 对象")
    ...
    windows = _usage_windows(data, rate_limit)
    plan_type = str(data.get("plan_type") or rate_limit.get("plan_type") or claims.get("claim_plan_type") or "")

    short_window = next((window for window in windows if _window_kind(window) == "short"), None)
    weekly_window = next((window for window in windows if _window_kind(window) == "weekly"), None)
    monthly_window = next((window for window in windows if _window_kind(window) == "monthly"), None)
    if plan_type.lower() == "free" and weekly_window is None:
        weekly_window = primary or secondary
        short_window = None

    def window_fields(prefix: str, window: dict | None) -> dict:
        if not window:
            return {}
        return {
            f"{prefix}_used_percent": window.get("used_percent"),
            f"{prefix}_reset_at": window.get("reset_at"),
            f"{prefix}_reset_after_seconds": window.get("reset_after_seconds"),
            f"{prefix}_limit_window_seconds": window.get("limit_window_seconds"),
        }
    ...
    result = {
        ...
        "additional_rate_limits_count": len(data.get("additional_rate_limits") or []),
    }
    result.update(window_fields("short", short_window))
    result.update(window_fields("weekly", weekly_window))
    result.update(window_fields("monthly", monthly_window))
    result.update({k: v for k, v in claims.items() if k != "payload" and v is not None})
    return result


def _codex_usage_headers(env: BrowserSession, token: str, account_id: str) -> dict[str, str]:
    headers = env._get_common_headers()
    headers.update({
        "accept": "application/json",
        "authorization": f"Bearer {normalize_token(token)}",
        "chatgpt-account-id": str(account_id).strip(),
...
def check_codex_usage(
    ...
    *,
    ...
    """Query Codex usage, including weekly and monthly rate-limit windows."""
    token = normalize_token(token)
    if not token:
        return {"ok": False, "checked_at": now_iso(), "error": "AT 为空", "needs_live_check": True}
    claims = token_claims(token)
    account_id = str(account_id or claims.get("account_id") or "").strip()
    if not account_id:
        return {"ok": False, "checked_at": now_iso(), "error": "Codex 凭证缺少 account_id"}
    if claims.get("token_expired") is True:
        return {
            "ok": False,
            "checked_at": now_iso(),
            "error": "AT 已过期，需要使用 RT 刷新",
    ...
    try:
        ...
        return {"ok": False, "checked_at": now_iso(), "error": f"Codex 周限额网络配置错误: {exc}"}
    route_meta = {k: v for k, v in route.items() if k != "proxy"}
    env = None
    try:
        env = BrowserSession(proxy=route["proxy"], detect_exit_geo=False)
        url = f"https://chatgpt.com{CODEX_USAGE_PATH}"
        response = env.get(
            url,
            headers=_codex_usage_headers(env, token, account_id),
            allow_redirects=False,
            timeout=timeout_seconds,
        )
        http_status = int(response.status_code)
        if not 200 <= http_status < 300:
            needs_refresh = http_status == 401
            return {
                "ok": False,
                "checked_at": now_iso(),
                "http_status": http_status,
                "error": "AT 已过期，需要使用 RT 刷新" if needs_refresh else f"Codex 周限额查询 HTTP {http_status}",
                "needs_live_check": needs_refresh,
                **route_meta,
            }
        payload = response.json()
        parsed = parse_codex_usage(payload, token=token)
        parsed.update({"http_status": http_status, **route_meta})
        return parsed
    except Exception as exc:
        logger.debug("Codex 周限额查询失败: %s: %s", type(exc).__name__, exc, exc_info=True)
        return {
            "ok": False,
            "checked_at": now_iso(),
