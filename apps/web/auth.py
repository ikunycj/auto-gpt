...
from flask import Response, jsonify, redirect, request, send_file, session, url_for
...
def register_auth_routes(app: Any) -> None:
    ...
    def _auth_login():
        error = "授权码错误" if request.args.get("error") == "invalid" else ""
        ...
        if request.method == "POST":
            ...
            error = "授权码错误"
            react_index = Path(str(app.config.get("REACT_INDEX_PATH") or ""))
            if react_index.is_file():
                return redirect(url_for("auth_login", next=next_url, error="invalid"))
        react_index = Path(str(app.config.get("REACT_INDEX_PATH") or ""))
        if request.method == "GET" and react_index.is_file():
